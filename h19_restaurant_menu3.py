...
class Menu:
    ...
    def __str__(self):
        ...
        for i, dish in enumerate(self.food.values()):
            menu[i].extend(
                d.name for d in sorted(dish, key=lambda d: d.preparation_time)
            )
    ...
    def get_minimum_preparation_time(self):
        return sum(
            min((d.preparation_time for d in dish), default=0)
            for dish in self.food.values()
        )

    def get_maximum_preparation_time(self):
        return sum(
            max((d.preparation_time for d in dish), default=0)
            for dish in self.food.values()
        )
