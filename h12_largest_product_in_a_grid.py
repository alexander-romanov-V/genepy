"""HARD - Largest product in a grid
...
e = [list(map(int, l.split())) for l in s.splitlines()]
...
def solution3():
    ...
    def get_segments(x, y, n=4):
        directions = [(1, 0), (0, 1), (1, 1), (1, -1)]
        for dx, dy in directions:
            if 0 <= x + (n - 1) * dx < w and 0 <= y + (n - 1) * dy < h:
                yield [(x + i * dx, y + i * dy) for i in range(n)]

    cases = itertools.product(range(h), range(w))
